...
首页在每次重新进入时交替播放原开场影片与 AE 合集，默认静音并铺满屏幕。页面含固定进度线、由滚动位置驱动的横向胶片轨道、错位作品布局，以及跟随鼠标局部显现的网格。桌面端使用白色镂空圆形光标；移动设备使用原生触控。邮箱 `titian0507@gmail.com` 与微信 `titian1231` 可以点击复制。个人照片是 `assets/portrait.jpg`。
